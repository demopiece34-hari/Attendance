...
from utils import now_ist, safe_float, safe_json_dumps, safe_json_loads, safe_strip, timestamp_str, today_str
...
def _service_account_dict() -> dict:
    ...
    raise RuntimeError(
        ...
        "Use [gcp_service_account] or [gcp] in .streamlit/secrets.toml"
    )
...
def ensure_structure() -> None:
    ...
    for sheet_name, headers in ws_map.items():
        ...
        if existing != headers:
            if existing:
                # If the sheet already has data but different headers, do not overwrite silently.
                continue
...
def _update_cell_range(sheet_name: str, row_index: int, values: List[str]) -> None:
    ws = _worksheet(sheet_name)
    end_col = chr(64 + len(values)) if len(values) <= 26 else None
    if end_col:
        ws.update(f"A{row_index}:{end_col}{row_index}", [values])
    else:
        # Fallback for wider tables
...
def upsert_setting(key: str, value: str) -> None:
    ws = _worksheet(SETTINGS_SHEET)
    row_idx = _find_row_by_key(SETTINGS_SHEET, "key", key)
    data_row = [key, value, timestamp_str()]
    if row_idx:
        _update_cell_range(SETTINGS_SHEET, row_idx, data_row)
    else:
        _append_row(SETTINGS_SHEET, data_row)


def _row_to_dict(headers: List[str], row: Dict) -> Dict:
    out = {}
    for h in headers:
        out[h] = row.get(h)
    return out
...
def upsert_student(record: Dict) -> None:
    ...
    row = [
        ...
        safe_strip(record.get("allowed_radius_m", DEFAULT_ALLOWED_RADIUS_M)),
    ...
    if row_idx:
        _update_cell_range(STUDENTS_SHEET, row_idx, row)
    else:
...
def upsert_staff(record: Dict) -> None:
    ...
    if row_idx:
        _update_cell_range(STAFF_SHEET, row_idx, row)
    else:
...
def get_present_pending_absent(date_value: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def create_default_settings_if_missing() -> None:
    defaults = {
        ...
        SETTING_ALLOWED_RADIUS_M: str(DEFAULT_ALLOWED_RADIUS_M),
    }
